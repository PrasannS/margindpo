...
def train(args):
    ...
    # configure tokenizer
    tokenizer = get_tokenizer("EleutherAI/pythia-2.8b" if "pythia" in args.pretrain else args.pretrain, model.model, "right", strategy, use_fast=not args.disable_fast_tokenizer)
    ...
    get_tokenizer("EleutherAI/pythia-2.8b" if "pythia" in args.pretrain else args.pretrain, ref_model.model, "right", strategy, use_fast=not args.disable_fast_tokenizer)
    ...
    # )
    fulldata = DatasetDict.load_from_disk(args.dataset)
    train_data = fulldata['train']
    eval_data = fulldata['test']
